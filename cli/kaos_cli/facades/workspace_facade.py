...
class WorkspaceFacade:
    ...
    def info(self):
        ...
        # GET /workspace/<name>
        r = requests.get(f"{base_url}/workspace/{name}", headers={"Authorization": f"Bearer {self.token}"})
    ...
    def delete(self):
        ...
        r = requests.delete(f"{base_url}/workspace/{name}", headers={"Authorization": f"Bearer {self.token}"})
        if r.status_code >= 300:
            raise RequestError(r.text)
    ...
    def list(self, as_dict=True):
        ...
        # GET /workspace
        r = requests.get(f"{base_url}/workspace", headers={"Authorization": f"Bearer {self.token}"})
        if r.status_code >= 300:
            raise RequestError(r.text)
